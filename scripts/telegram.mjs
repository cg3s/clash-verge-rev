...
async function sendTelegramNotification() {
  ...
  const downloadUrl =
    ...
    `https://github.com/cg3s/clash-verge-rev/releases/download/v${version}`;
  ...
  const content = `<b>🎉 <a href="https://github.com/cg3s/clash-verge-rev/releases/tag/autobuild">Clash Max v${version}</a> ${releaseTitle}</b>\n\n${formattedContent}`;
    ...
    await axios.post(
      `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
      {
        ...
        link_preview_options: {
          is_disabled: false,
          url: `https://github.com/cg3s/clash-verge-rev/releases/tag/v${encodedVersion}`,
